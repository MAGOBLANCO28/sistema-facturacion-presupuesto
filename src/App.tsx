...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex text-zinc-900 font-sans">
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white border-r border-zinc-200 transition-all duration-300 flex flex-col fixed h-full z-20`}>
        <div className="p-6 flex flex-col items-center gap-4">
          ...
          {isSidebarOpen && (
            <div className="flex flex-col items-center text-center">
              ...
              <h1 className="font-black text-sm tracking-widest text-red-700 leading-none">Faktio</h1>
              <p className="text-[7px] uppercase tracking-tighter font-bold text-zinc-400 mt-1">Facturación profesional</p>
            </div>
          )}
        </div>
          ...
          <div className={`flex items-center gap-3 ${!isSidebarOpen && 'justify-center'}`}>
            ...
            {isSidebarOpen && (
              <div className="flex-1 overflow-hidden">
                <p className="text-xs font-semibold truncate">Mi cuenta</p>
                <p className="text-[10px] text-zinc-500 truncate">Administrador</p>
              </div>
            )}
            <button onClick={handleLogout} className="p-1.5 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Cerrar sesión">
              <LogOut size={16} />
            </button>
          </div>
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'history' && (
              <motion.div key="history" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <HistoryView onEdit={(doc) => { setSelectedDoc(doc); setDocType(doc.type); setView('editor'); }} onPreview={(doc) => { setSelectedDoc(doc); setView('preview'); }} />
              </motion.div>
            )}
            {view === 'settings' && (
...
function SidebarItem({ icon, label, active, onClick, collapsed }: { icon: React.ReactNode, label: string, active: boolean, onClick: () => void, collapsed: boolean }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${active ? 'bg-red-50 text-red-700 font-medium' : 'text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900'} ${collapsed ? 'justify-center' : ''}`} title={collapsed ? label : ''}>
